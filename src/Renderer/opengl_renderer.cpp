...
void setup_gl_renderer()
{
	...
	glGenFramebuffers(1, &framebuffer_post);
	glBindFramebuffer(GL_FRAMEBUFFER, framebuffer_post);

	// generate texture

	glGenTextures(1, &texColorBuffer_post);
	...
	// attach it to currently bound framebuffer object
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texColorBuffer_post, 0);
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
		std::cout << "ERROR::FRAMEBUFFER:: Framebuffer is not complete!" << std::endl;
